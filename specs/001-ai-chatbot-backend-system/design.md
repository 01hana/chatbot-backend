...
**版本**：1.8.0 | **建立日期**：2026-04-09 | **狀態**：Draft  
**承接文件**：`spec.md` v1.6.0  
...
> **本期實作總原則**：本期後端聊天主流程採 SSE / streaming 串接 OpenAI；前端以 `sessionToken` 作為匿名訪客會話識別，後端內部映射至 `sessionId`。本期正式提供 Widget Config API。本期正式納入 Dashboard、Feedback、Ticket。本期不做 Auth / Login / RBAC、Email 通知、handoff status API。
...
| DG-2 | 外部 LLM Provider（本期為 OpenAI）可替換，業務邏輯不依賴特定 SDK；LLM 是核心回覆生成器，但不是自由回答器 |
...
│  │   LlmProvider (OpenAI)  │  EmailProvider           │  │
...
└─────────────────────────────────────────────────────────┘
        │                    │                  │
   ┌────▼─────┐    ┌─────────▼──────┐  ┌───────▼─────────────┐
   │ Postgres │    │  OpenAI API    │  │  External Webhook    │
   │ (主資料) │    │  (LLM 生成)    │  │  (active 本期啟用)   │
   └──────────┘    └────────────────┘  │  SMTP (future,       │
                                       │   本期不啟用)         │
                                       └─────────────────────┘
```
...
### KDD-3：LLM Provider 抽象化，OpenAI 為本期核心回覆生成器

**選擇**：定義 `ILlmProvider` 介面，本期以 `OpenAiProvider` 實作（OpenAI Chat Completions API）；透過 NestJS DI 注入。  
**理由**：業務邏輯只依賴介面，LLM 替換不影響 Pipeline。**LLM 是主回覆生成器，但不是自由回答器**：只在 RAG 信心分數達閾值後才可呼叫，prompt 由 `PromptBuilder` 嚴格控制 context 範圍。LLM 回應需記錄 token 用量、回覆時間與 model 資訊，供成本追蹤與異常分析使用。
...
`ILlmProvider` 介面新增 `stream()` 方法，供 `OpenAiProvider` 實作 OpenAI streaming 呼叫（`stream: true`）。
...
| `LlmModule` | LLM Provider 抽象 + OpenAI 實作（含 streaming）、Prompt Builder | ✅ 本期 |
...
- 呼叫 `ILlmProvider.stream()`（OpenAI streaming API，`stream: true`）
...
- **前端斷線偵測**：NestJS SSE 可透過 `res.on('close', callback)` 感知；觸發後呼叫 `AbortController.abort()` 中止 OpenAI streaming 呼叫（避免浪費 token）
...
> 本期實作：`OpenAiProvider`（使用 `openai` npm 套件）。Provider 透過 NestJS DI 注入，以 `LLM_PROVIDER` token 提供。  
> `stream()` 方法對應 OpenAI streaming API（`stream: true`），回傳 `AsyncIterable<LlmStreamChunk>`，供 SSE Controller 逐步推送。
...
1. pgvector 需額外安裝擴充，且 embedding 生成需呼叫 OpenAI Embeddings API，增加複雜度
...
| Model | `LLM_MODEL` | 預設 `gpt-4o-mini` |
| Max Tokens | `LLM_MAX_TOKENS` | 預設 `1000` |
| Timeout | `LLM_TIMEOUT_MS` | 預設 `10000` |
| Base URL | `LLM_BASE_URL` | 可替換為 Azure OpenAI 或其他相容端點 |

**Data Opt-out**：OpenAI 組織設定需確認關閉訓練資料使用（API 呼叫預設不用於訓練，但需確認）。
...
本專案核心依賴 OpenAI API；LLM 呼叫的成本、效能與異常須可追蹤。本期透過 **AuditLog** 作為 LLM 觀測層：
...
| 設定類型 | 權威來源 | 範例 |
|---------|---------|------|
| **Secrets / Credentials** | 環境變數（`.env` / 容器 env） | `OPENAI_API_KEY`, `DATABASE_URL`, `WEBHOOK_SECRET` |
...
|------|------|------|
| 資料庫 | `DATABASE_URL` | 必填 |
| LLM | `OPENAI_API_KEY`, `LLM_MODEL` | 必填 |
...
| LLM 替換（非 OpenAI） | 任何時間 | `ILlmProvider` 介面 |
