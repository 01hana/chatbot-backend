...
**主流程：**
訪客詢問適合的型材 → 後端偵測為推薦意圖 → 後端依固定順序追問：①用途（purpose）→ ②材質（material）→ ③長度（length）→ ④使用環境（environment）→ 收斂到足夠資訊後 → 比對規格知識庫 → 提供推薦清單 + 規格摘要 + 來源 → 偵測高意向 → 主動推動留資

**關鍵限制：**
- 不可在問診不足時直接推薦
- 規格比對規則優先；LLM 僅用於摘要與呈現，不可自由生成規格內容
- 本期問診欄位固定為四項，追問順序固定（purpose → material → length → environment）
...
| ID | 需求描述 | 優先級 |
|----|----------|--------|
| FR-020 | 當偵測到產品推薦意圖時，系統需先進行問診，依固定順序收集四項資訊：①用途（purpose）→ ②材質（material）→ ③長度（length）→ ④使用環境（environment） | P1 |
...
| Q-004 | 本期問診模板固定為四欄位（purpose / material / length / environment），追問順序已拍板；問診需涵蓋哪些產品線，由甲方確認 | FR-020 實作範圍 | 甲方 |
