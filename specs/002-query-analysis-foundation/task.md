...
- [ ] `GET /api/v1/admin/system-config` 回傳所有 SystemConfig entries（JSON array）
- [ ] `PATCH /api/v1/admin/system-config/rag_minimum_score` 更新值後，下一次 `SystemConfigService.getNumber('rag_minimum_score')` 立即反映（不需重啟）
- [ ] Unit tests：findAll / upsert / cache invalidation
- [ ] `NotImplementedException` 全數移除
...
- [ ] POST 建立後，`IntentService.getCachedTemplates()` 包含新 template（下一次 detect 可用）
- [ ] DELETE（停用）後，`IntentService.detect()` 不再使用此 template
- [ ] Unit tests：CRUD + cache invalidation
- [ ] Admin module wired 進 `AdminModule`
...
- [ ] POST 建立後，`IntentService.getCachedGlossary()` 立即包含新 term
- [ ] DELETE 後，`getCachedGlossary()` 不再有此 term
- [ ] Unit tests：CRUD + cache invalidation
- [ ] Admin module wired 進 `AdminModule`
...
- [ ] admin-intent service spec：≥ 8 個測試通過
- [ ] admin-glossary service spec：≥ 8 個測試通過
