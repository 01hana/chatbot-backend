
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/SystemConfig.js'
export type * from './models/SafetyRule.js'
export type * from './models/BlacklistEntry.js'
export type * from './models/IntentTemplate.js'
export type * from './models/GlossaryTerm.js'
export type * from './models/KnowledgeEntry.js'
export type * from './models/KnowledgeCategory.js'
export type * from './models/KnowledgeVersion.js'
export type * from './models/Conversation.js'
export type * from './models/ConversationMessage.js'
export type * from './models/AuditLog.js'
export type * from './models/QueryRule.js'
export type * from './models/KnowledgeDocument.js'
export type * from './models/KnowledgeChunk.js'
export type * from './models/KnowledgeEntity.js'
export type * from './models/KnowledgeRelation.js'
export type * from './models/Lead.js'
export type * from './models/Ticket.js'
export type * from './models/Feedback.js'
export type * from './commonInputTypes.js'